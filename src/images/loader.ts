const isTransformAndExtract =
  /^tr:w-(\d+):w-(\d+),h-(\d+),cm-extract,y-(\d+),x-(\d+)$/
const isTransformOnly = /^tr:w-(\d+),h-(\d+)$/
const isMedia =
  /^(https:\/\/ik.imagekit.io\/passvisitwallonia\/)(tr:[^/]+)?\/?(media\/.+)$/
...
function updateTransformation(
  ...
  const applyRatio = (value: number, ratio: number): number =>
