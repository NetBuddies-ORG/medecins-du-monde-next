'use client'
import {GetOrganismesQuery, Organisme, OrganismeEntity} from "@/services/GraphQL";
...
export function Organizations({extraData}: OrganizationsProps) {

    const [keyword, setKeyword] = useState<string>('');
    const [organismes, setOrganismes] = useState<any[]>(extraData.organismes.data);

    const {getOrganismes, isReady} = useDBIndex('fr');
        ...
        <>
            <div className="page-container">
                <div className='searchbar input'>
                    <FaSearch />
                    <input type='text' value={keyword} onChange={(value) => setKeyword(value.target.value)} placeholder='Entrez un mot clé ..' />
                </div>
                <div className='card-container-organisme'>
                    {
                        organismes.map(item => <Link key={item.id} href={item.attributes.Nom}>
