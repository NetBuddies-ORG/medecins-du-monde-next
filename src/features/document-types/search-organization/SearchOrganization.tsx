...
export function SearchOrganization({
  ...
  const [categoriesForFilterDisplay, setCategoriesForFilterDisplay] = useState<
    (CategorieEntity & {
      id: string
    })[]
  >([])
  ...
  useEffect(() => {
    if (isReady) {
      ...
      categoriesInit.forEach((item) => {
        subIdsInitToAdd.push(
          ...item.attributes.sous_categories.data.map((item) => item.id)
        )
      })

      const categoriesFromSubCategories = categories.categories.data.filter(
        (item) => {
          return item.attributes.sous_categories.data.some((item) =>
            searchParams.getAll('subCategories').includes(item.id)
          )
        }
      )
  ...
  function toggleAccordions(category: CategorieEntity) {
    ...
    if (selectedOpenCategories.includes(category.id)) {
      setSelectedOpenCategories(
        selectedOpenCategories.filter((item) => item !== category.id)
      )
    } else {
  ...
  function handleCategoryCheck(
    ...
    if (
      !category.attributes.sous_categories.data
        .flatMap((sc) => sc.id)
        .some((id) => authorizedSubCategories.includes(id))
    ) {
      return
    }
    ...
    if (subCategories?.length) {
      if (
        subCategories.every((item) => selectedSubCategories.includes(item.id))
      ) {
        setSubSelectedCategories(
          selectedSubCategories.filter(
            (item) => !subCategories.map((item) => item.id).includes(item)
          )
        )
      } else {
        setSubSelectedCategories([
          ...
          ...subCategories
            .filter((sc) => authorizedSubCategories.includes(sc.id))
            .map((item) => item.id),
        ])
      }
    } else {
  ...
  const positionCenter = (organismes: any) => {
    ...
    if (latCenter && longCenter) {
      setCenterCoordinates({
        latitude: latCenter / count,
        longitude: longCenter / count,
      })
    }
  }
  ...
  function isCategoryChecked(category: CategorieEntity) {
    ...
    return category.attributes.sous_categories?.data?.every((item) =>
  ...
  function isPartiallyChecked(category: CategorieEntity) {
    ...
    return category.attributes.sous_categories?.data?.some((item) =>
      selectedSubCategories.includes(item.id)
    )
  }
    ...
    <>
      <AnimatePresence>
        <motion.div className="page-container">
          {
            <div
              className={'slideout-results' + (isSlideOutOpen ? ' isOpen' : '')}
              id="slideout-results"
            >
                  ...
                  <div className="searchbar">
                    <div className="searchbar__input input">
                      ...
                      >
                        ...
                        {publics.publicSpecifiques.data.map((item) => (
                          ...
                          <div
                            ...
                            className={
                              ...
                              (!category.attributes.sous_categories.data
                                .flatMap((sc) => sc.id)
                                .some((id) =>
                                  authorizedSubCategories.includes(id)
                                )
                                ? ' disabled'
                                : '')
                            }
                          >
                            ...
                            >
                              <span
                                ...
                                onClick={(event) =>
                                  handleCategoryCheck(event, category)
                                }
                              >
                                {isCategoryChecked(category) ? (
                                  <FaCheck />
                              ...
                              <span>
                                ...
                                <span className="angle">
                                  {category.attributes.sous_categories.data
                                    ?.length ? (
                                    selectedOpenCategories.includes(
                                      category.id
                                    ) ? (
                                      <FaAngleUp />
                                    ) : (
                                ...
                                (subCategory) => {
                                  return (
                                    <li
                                      ...
                                      className={
                                        'sub-category' +
                                        (authorizedSubCategories.includes(
                                          subCategory.id
                                        ) ||
          ...
          <div className="resultbar">
            <span className="count">{organismes.length} résultats</span>
            <div className={'view-picker'}>
              <span
                className={'icon-picker' + (isMapViewSelect ? '' : ' active')}
          ...
          {isMapViewSelect ? (
            <>
              <div className="map-container">
                {centerCoordinates && (
                  <LeafletMap coordinates={centerCoordinates} zoom={10}>
                    {organismes.length > 0 &&
                    ...
                    return (
                      <Link
                        ...
                        href={
                          ...
                          '/' +
                          organisme.generatedUrl
                        }
                      >
                        <div className="card organisme">
                          <div className="up">{organisme.Nom}</div>
                          <div className="department">
                            {organisme?.Departement && (
                              <span>
                                <FaBuildingUser /> {organisme?.Departement}
                              </span>
                            )}
                          </div>
                          <div className="down">
