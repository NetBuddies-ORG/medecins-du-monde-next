...
export function CardList({
  ...
  const authorizedCategories = categories.data.map((item) => item.id)
  ...
  function handleSelectedCategories(id: string) {
    if (healthId === id) {
      if (
        subHealthIds.every((subHealthId) =>
          selectedCategories.includes(subHealthId)
        )
      ) {
  ...
  return (
    <>
      ...
      <div className="searchbar">
        {isInfoVisible && (
          <div className="info">
            ...
            <h2>
              {' '}
              {t('HOME_HELP_WITH_TOOLS')}{' '}
              <a onClick={() => showModal(true)}>{t('HOME_CLICK_HERE')}</a>
            </h2>
          </div>
        )}
        <div className="warn">
          <FaTriangleExclamation />
          <h2>
            Le site est en construction, les données ne sont donc pas complètes
          </h2>
        </div>
        ...
        <div className="searchbar__input input">
          <select
            ...
            className={
              selectedPublics === '0' || !selectedPublics
                ? 'disabled-select'
                : ''
            }
            onChange={handlePublicsChange}
          >
            <option value="0" className="disabled-option">
      ...
      <div className="card-container">
        <Link
          prefetch={false}
          href={extraData?.home?.data?.attributes?.UrgencesLink}
        >
          <motion.div
          ...
          >
            ...
            <div className="card__title">
              <span>{t('HOME_URGENCES')}</span>
            </div>
          </motion.div>
        </Link>
        <Link
          prefetch={false}
          href={extraData?.home?.data?.attributes?.OrientationsLink}
        >
          <motion.div
            whileHover={{ scale: 1.05 }}
            transition={{ duration: 0.1 }}
        ...
        {categories.data.map((category) => {
          ...
          return (
            <motion.div
              ...
              className={
                'card' +
                (!authorizedCategories.includes(category.id) &&
                !selectedCategories.includes(category.id)
                  ? ' disabled'
                  : '') +
                (selectedCategories.includes(category.id) ||
                (category.id === healthId &&
                  subHealthIds.every((subHealthId) =>
                    selectedCategories.includes(subHealthId)
                  ))
                  ? ' isSelected'
            ...
            >
              ...
              <div className="card__title">
                <span>{category?.attributes.Nom}</span>
              </div>
            </motion.div>
          )
        })}
      ...
      <div
        className={
          'footer-search' + (selectedCategories.length <= 0 ? ' isHidden' : '')
        }
      >
        <Link
          prefetch={false}
        ...
        >
          <button className="btn btn-primary">Rechercher un organisme</button>
        </Link>
      </div>
    </>
  )
