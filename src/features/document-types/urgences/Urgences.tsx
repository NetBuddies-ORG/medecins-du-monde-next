...
export function Urgences({ extraData }: UrgencesProps) {
    ...
    const categories = extraData.urgence.data.attributes.Element?.map(e => e.c_o_u);
    const uniqueCategories = [...new Set(categories?.map(c => c?.data?.id))];

    const urgencesInCategories = uniqueCategories.map(categoryId => {
        return {
            category: categories?.find(c => c?.data?.id === categoryId),
            orientations: extraData.urgence.data.attributes.Element?.filter(e => e.c_o_u?.data?.id === categoryId)
        }
        ...
        <div className='page-container'>
            <div className="urgences-container">
                {urgencesInCategories?.map(category => {
                    return <React.Fragment key={`${category?.category?.data?.id}_categorykey`}>
