...
export async function DocumentTypes() {
  ...
  return (
    <>
      <CustomHeader />
      <main>{await displayContent(pages, language)}</main>
      <CustomFooter />
    </>
  )
}
  ...
  switch (page.data[0].attributes.ContentType) {
    ...
    case 'Organizations':
      return (
        <Suspense>
          <Organizations
            extraData={await getOrganizationsPage(language)}
            languague={language}
          />
        </Suspense>
      )
    ...
    case 'Services':
      return (
        <>
          <ServicesPage
            language={language}
            extraData={await getServicesPage(language)}
          />
        </>
      )
    case 'Urgences':
...
const getOrganizationsPage = cache(async function getOrganizationsPage(
  language: string
) {
  const client = getStrapiClient()
...
const getOrientationsPage = cache(async function getOrientationsPage(
  language: string
) {
  const client = getStrapiClient()
